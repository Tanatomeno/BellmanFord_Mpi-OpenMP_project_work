...
#include "toInclude/bellmanFordSerial.h"
...
/*BellmanFord--------------------------------------------------------*/
    ...
    negativeCicle= bellmanFord_serial2D(graph, V, sourceVertex,  dist, parent);
...
/*print BellmanFord----------------------------------------------------------*/
    ...
    if(negativeCicle == 0){
        print_bellmanFord_serial_txt(fileName2, V, dist, parent);
        #ifdef TESTING
            if(V<50){
                print_bellmanFord_serial(V, dist, parent);
            }else{
